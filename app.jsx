...
const GAMES = [
  ...
  { id: "workflow-wiring", title: "Workflow Wiring", cluster: "workflows-cluster", desc: "Connect event triggers to automated actions in a logic puzzle.", summary: "Concept takeaway: Zuper's real workflow automation connects triggers to actions behind the scenes.", docsConcept: "Custom Workflows and Onsite Checklists", docsBlurb: "teams connect triggers to automated actions and required onsite steps" },
  { id: "system-stabilizer", title: "System Stabilizer", cluster: "core-platform", desc: "Resource-management mini-game. Keep every system meter in range.", summary: "Concept takeaway: Zuper's real platform monitors and balances system load automatically.", docsConcept: "Dashboard", docsBlurb: "the Dashboard module surfaces key metrics from every module in one place, so nothing drifts out of range unnoticed" },
  { id: "pipe-flow", title: "Pipe Flow", cluster: "data-pipeline", desc: "Rotate-the-segment puzzle. Connect the source to the target before time runs out — grows each level.", summary: "Concept takeaway: Zuper's real data pipeline moves information between systems automatically, already connected.", docsConcept: "Transfer Orders", docsBlurb: "the Transfer Orders module streamlines moving inventory between locations, source to destination" },
  { id: "spinning-plates", title: "Spinning Plates", cluster: "command-center", desc: "Real-time survival. Ping each system before it goes critical — more come online the longer you last.", summary: "Concept takeaway: Zuper's real command center monitors every system at once so nothing goes critical unnoticed.", docsConcept: "the Dispatch Board", docsBlurb: "it gives a centralized view of every job — including which ones are unscheduled, unassigned, or turning overdue — before anything goes critical" },
...
/* Every game's "Skip & Read Summary" used to show the exact same canned disclaimer
   ...
   hasn't loaded yet. Direct follow-up request: dropped the trailing "this is an
   illustrative analogy, not a simulation" boilerplate entirely (every game's own
   description already makes clear it's a mini-game — this line was just noise on top
   of it) so the takeaway reads as a clean statement of the real concept instead of
   ending on a hedge. */
...
function Taskbar({ onStartClick, running, onRunningClick, theme }) {
  ...
  return (
    <div className="fixed left-0 right-0 bottom-0 h-[52px] flex items-center gap-3 px-3 z-[800] border-t" style={{ background: "linear-gradient(180deg, rgba(255,255,255,.14) 0%, rgba(110,100,96,.32) 100%)", backdropFilter: "blur(28px) saturate(1.7)", WebkitBackdropFilter: "blur(28px) saturate(1.7)", borderColor: "rgba(255,255,255,.24)", boxShadow: bevel("out-shallow", t.winBorder) + ", inset 0 1px 0 rgba(255,255,255,.12)" }}>
